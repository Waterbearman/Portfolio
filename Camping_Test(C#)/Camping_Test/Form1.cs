            ...
            else
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        dataGridView1.DataSource = DataManager.bookings;
                        DataManager.Save();
                        MessageBox.Show($"{booking.Area}을 {user.Name}님께 예약 되었습니다.");
                    }
                }
                catch (Exception)
                {
                    MessageBox.Show("존재하지 않는 구역 또는 사용자 입니다.");
                }
            }
        }
                ...
                try
                {
                    ...
                    if (booking.Checking)
                    {
                        ...
                        if(timeDiff.Days > 7)
                            ...
                            MessageBox.Show(booking.Area + "정상 체크아웃");
                    }
                    else
